...
def render_report_pdf(report: DueDiligenceReport) -> bytes:
    ...
    for breakdown in report.score_breakdowns:
        rows = [[f"{breakdown.category}: {breakdown.score} / 100", f"Confidence: {breakdown.confidence}"]] + [[f"+{item.points} {item.label}", item.note] for item in breakdown.contributing_factors] + [[f"{item.points} {item.label}", item.note] for item in breakdown.deductions]
